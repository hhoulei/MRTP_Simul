...
DataGenerator_wald <- function(N1,N2,N3,N4,g,p1,p2,beta1,beta2,sigma_x1,sigma_x2,
  ...
  prob_Y1 <- plogis(beta1 * X3 + rnorm(N3, 0, sqrt(sigma2_epi1)))
  Y1 <- rbinom(N3, 1, prob_Y1)
  
  # 生成Y2的概率
  prob_Y2 <- plogis(beta2 * X4 + rnorm(N4, 0, sqrt(sigma2_epi2)))
  Y2 <- rbinom(N4, 1, prob_Y2)
...
Comp_wald <- function(N1b,N2b,N3b,N4b,gb,p1b,p2b,beta1b,beta2b,sigma_x1b,sigma_x2b,
  ...
  for(j in 1:gb){
    ...
    FF3 <- glm(fdata1$Y1 ~ fdata1$G3[,j], family=binomial)
    betaY1G <- c(betaY1G, FF3$coef[2])
    seY1G <- c(seY1G, summary(FF3)$coef[2,2])
    
    FF4 <- glm(fdata1$Y2 ~ fdata1$G4[,j], family=binomial)
...
bb <- c(0,0.05,0.1,0.15,0.2)
rho_bb <- c(0.1,seq(0.2,0.8,0.2),0.9)
siga1b <- c(0,0,-0.2)
siga2b <- c(0,0.2,0.2)
    ...
    for(t in 1:length(rho_bb)){
      for(p in 1:length(siga1b)){
        ...
        write.csv(result,paste0("MC","_N2_",N2b,"_N4_",N4b,"_g_",gb,
                                "_beta1_",b1,"_beta2_",b2,"_rho_b_",
                                rho_b,"_siga1_",siga1,"_siga2_",siga2,
                                "_0130.csv"))  
